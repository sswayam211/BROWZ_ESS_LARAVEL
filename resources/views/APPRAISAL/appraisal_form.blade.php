<div>
    ...
    <form action="save_appraisal_data_by_sup" id="appraisal-form-submit" method="post">
        @csrf
        <div class="row">
            ...
            <div class="col-md-4 px-2 mb-3">
                <label for="EVAL_DATE" class="form-label">Appraisal Date<span
                        class="text-danger">*</span></label>
                <input type="text" class="form-control shadow-none" id="EVAL_DATE" name="EVAL_DATE" value="{{ date('d-M-Y') }}" readonly required>
            </div>
        </div>
        <hr>
        ...
        @if($KPI_CODE && !$FINAL_SUBMIT && !$IS_SUBMITED_ALREADY)
        ...
        @elseif($KPI_CODE && ($FINAL_SUBMIT || $IS_SUBMITED_ALREADY))
        ...
        <div class="text-end mt-4">
            <button type="submit" class="button rounded-0 mb-4" id="SAVE_APPRAISAL_DATA" name="SUBMIT_APPRAISAL">Submit</button>
        </div>
        @endif

        @elseif(empty($KPI_CODE))
